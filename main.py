...
default frozen policy locks observations to the first origin when later SCADA
is unavailable; its growing context age is recorded in each output audit.
"""
...
def simulation_origins(start: str, end: str, simulation_timezone: str = "UTC") -> list[pd.Timestamp]:
    ...
    return list(pd.date_range(first, last, freq="D", tz=simulation_timezone).tz_convert("UTC"))
...
def build_training_arrays(
    ...
    *,
    ...
    history_arrays, weather_arrays, labels, origins, audits = [], [], [], [], []
    skipped = 0
...
def run(args: argparse.Namespace) -> None:
    ...
    if not args.demo:
        if not args.data_timezone or not args.turbine_1 or not args.turbine_2:
            raise ValueError("Real execution requires --turbine-1, --turbine-2 and --data-timezone.")
        # Lazy imports keep --help and --demo independent of PyTorch, CUDA and RAPIDS.
        ...
        manifest["input_files"] = [str(Path(args.turbine_1).resolve()), str(Path(args.turbine_2).resolve())]
    ...
    for origin in origins:
        if args.demo:
            ...
            if frozen_state is not None:
                histories, batch, context, model, metrics, checkpoint = frozen_state
            else:
                histories = [
                    data_agent.load_history(Path(path), current_date=context_origin)
                ...
                if not np.isfinite(context).all():
                    raise ValueError(f"Incomplete inference history at {context_origin}; refusing to fill gaps.")
                model = ModelAgent(config)
                metrics = model.fit(batch.history, batch.weather, batch.targets)
                ...
                if args.history_policy == "frozen":
                    # Never advance SCADA or training labels when later observations are unavailable.
                    frozen_state = (histories, batch, context, model, metrics, checkpoint)
            weather, wind, forecast_audit = aligned_weather(forecasts_at(origin), origin, args.horizon)
            power = validate_power(model.predict(context, weather), wind)
            audit = {
                ...
                "history_policy_notice": (
                    "Frozen-context baseline: SCADA and labels fixed at first simulation origin. "
                    "Weather updates daily; increasingly stale context differs from training conditions."
                    if args.history_policy == "frozen" else
                    "Expanding history: observed SCADA required before every daily origin."
                ),
                ...
                "training": {
                    ...
                    "forecast_provenance": batch.weather_audits, "metrics": metrics,
                },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--turbine-1", type=Path, help="Turbine 1 source CSV (required unless --demo).")
    parser.add_argument("--turbine-2", type=Path, help="Turbine 2 source CSV (required unless --demo).")
    ...
    parser.add_argument(
        "--history-policy", choices=("frozen", "expanding"), default="frozen",
        help="Frozen locks SCADA and model at first origin (default); expanding needs new daily observations.",
    )
...
def main() -> None:
    ...
    if not args.demo and (not args.turbine_1 or not args.turbine_2 or not args.data_timezone):
        parser.error("real mode requires --turbine-1, --turbine-2 and --data-timezone")
    run(args)
