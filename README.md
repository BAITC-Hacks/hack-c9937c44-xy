...
Главный экран использует карту-схему и боковую панель в духе [Electricity Maps](https://app.electricitymaps.com/map/zone/DE/72h/fifteen_minutes): две выбираемые турбины, переключатель мощности/ветра и почасовая лента с воспроизведением. Выбор часа синхронизирован с графиками и таблицей. Работают дата выпуска, горизонт 24/48 часов, фильтр турбин и выгрузка CSV. Карта — условная схема, а не настоящая топография или погодный слой. Макет адаптируется к мобильному экрану.

Источник «Демо интерфейса» сразу показывает **синтетические** значения. «Python демо» читает синтетические CSV и JSON-аудит из `outputs/demo/`; «Прогноз модели» читает результаты `outputs/backtest/` после запуска пайплайна. Интерфейс не запускает GPU или погодный API. При отсутствии файла показывает ошибку и убирает старый прогноз. В режиме модели ветер указан для 10 м; пороги включения и отключения турбины к нему не применяются. Оценки точности появятся после получения фактических февральских данных.
...
При наличии `outputs/observations.csv` для модельного прогноза добавляются сопоставление с фактом, распределение ошибок и MAE/RMSE/bias. Без факта показатели качества остаются недоступными. Формат наблюдений, методика и CLI: [SCIENTIFIC_REPORTS.md](SCIENTIFIC_REPORTS.md).
...
Локально прошёл 41 тест: CPU-обучение и восстановление модели, хронология, доступность погоды, физические ограничения, сборка submission и оценка результатов. Также проверены обработка обоих исходных CSV, запрос 48 часов реального архива Open-Meteo для 01.02.2026 и демонстрационный цикл за 28 дней. CUDA/cuDF/cuML-ветка требует проверки на целевом кластере. Полный GPU-backtest и метрики скрытого февраля ещё не получены. Модельные тесты пропускаются при отсутствии PyTorch; для полного набора нужен `requirements.txt`.
