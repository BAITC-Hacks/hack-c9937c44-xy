...
При наличии `outputs/observations.csv` для модельного прогноза добавляются сопоставление с фактом, распределение ошибок и MAE/RMSE/bias. Без факта показатели качества остаются недоступными. Формат наблюдений, методика и CLI: [SCIENTIFIC_REPORTS.md](SCIENTIFIC_REPORTS.md). Раздел проверен с браузерным демо и артефактами Python; весь набор — **51 тест, OK**.
