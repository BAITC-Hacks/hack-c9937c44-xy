...
Start from the existing ALEM WIND prototype in this repository: preview/index.html, preview/styles.css, and preview/app.js. Keep its recognizable restrained forest-green/ivory identity, Russian interface copy, and basic controls, but improve the information hierarchy and visual quality. Create the new design as a separate runnable prototype so the existing preview remains available for comparison. Provide desktop (1440px) and mobile (390px) layouts, reusable visual tokens, and accessible interactions.
...
3. The main view is a large interactive hourly chart with two clearly distinguished lines: turbine T1 (51.04, 71.46) and T2 (51.05, 71.45). Values are normalized active power, 0–1 p.u.; label the unit. Shade daily boundaries lightly, show the exact UTC hour in tooltips, and make the date of forecast issue distinct from target time. Include a turbine filter. Below, place a smaller aligned wind-speed chart in m/s with a synchronized hover cursor and visible 3/25 m/s thresholds. Do not mix the two units on an unlabeled axis.
4. Beneath the charts, show two concise turbine cards, an expandable 48-row hourly table, and a small optional two-marker location map. The table includes valid hour, turbine, normalized forecast power, forecast wind speed, and any physical constraint applied. On mobile, retain the chart and provide a usable table alternative without tiny text.
