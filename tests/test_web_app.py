...
class WebTests(unittest.TestCase):
    ...
    def test_health_dashboard_and_private_file_boundaries(self):
        ...
        for path in ("/.env", "/web_app.py", "/preview/../.env", "/outputs/backtest/checkpoints/model.pt"):
            self.assertEqual(self.client.get(path).status_code, 404, path)

    def test_symlink_cannot_expose_an_existing_file_outside_root(self):
        with tempfile.TemporaryDirectory() as outside:
            secret = Path(outside) / "outside.html"
            secret.write_text("not-public")
            try:
                (self.root / "preview/leak.html").symlink_to(secret)
            except OSError as error:
                if getattr(error, "winerror", None) == 1314:
                    self.skipTest("Windows account lacks symlink creation privilege; tested on Linux CI")
                raise
            self.assertEqual(self.client.get("/preview/leak.html").status_code, 404)
